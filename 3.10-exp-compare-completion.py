...
def exp(data: list[int], useCompletion: bool) -> tuple[int, float, float]:
    result_path = "result.bin"
    result, encoding_time = period_encode_param(
        data, result_path, use_completion=useCompletion
    )
    start = time.time()
...
for file in files:
    ...
    # if dataset != "volt" and dataset != "dianwang":
    #     continue
    file_pull = os.path.join("data", file)
    df = pd.read_csv(file_pull)
    data = df["value"].tolist()
    for useCompletion in [True, False]:
        (result, encoding_time, decoding_time) = exp(data, useCompletion)
        exp_data["dataset"].append(dataset)
        exp_data["name"].append(file)
        exp_data["algorithm"].append("completion" if useCompletion else "direct")
        ...
        print(
            dataset,
            file,
            "completion" if useCompletion else "direct",
            result,
            encoding_time,
            decoding_time,
        )
...
exp_data_df.to_csv(
    os.path.join("exp_result", "exp_compare_completion.csv"), index=False
)
