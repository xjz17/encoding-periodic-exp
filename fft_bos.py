    ...
    for subdir, dirs, files in os.walk(root_dir):
        ...
        for file in files:
            if file.endswith('.csv'):
                ...
                data = df.iloc[:, 0].tolist()  # get data from the second column
                if len(data) < 1:
                    continue 
                ...
                print(f"Processed {file}: Compression Ratio = {compress_ratio}, Encoding Time = {encoding_time}")

    # Save experiment data to CSV
    exp_data_df = pd.DataFrame(data=exp_data)
    root_csv_dir = "/Users/xiaojinzhao/Documents/GitHub/encoding-outlier/icde0802/supply_experiment/R3O2_compare_compression/compression_ratio/fft_bos"
